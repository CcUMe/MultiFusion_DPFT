...
class KRadarProcessor():
    ...
    def prepare_sample(self, sample: Dict[str, str], description, dst: str) -> None:
        ...
        Arguments:
            sample: Dictionary with all file paths belonging to a
                    single sample.
            description: Scene description list [road_type, capture_time, climate].
            dst: Destination directory to save the processed sample data.
        """
        import json
        ...
        # 生成VLM文本（在description编码之前，使用原始description列表）
        # vlm_text_simple = self.generate_vlm_text(boxes, description, include_bbox_details=False)
        vlm_text_detailed = self.generate_vlm_text(boxes, description, include_bbox_details=True)
        # vlm_data_json = self.generate_vlm_data_json(boxes, description)

        # Encode description
        description_encoded = self.map_description(description)
        ...
        # Load lidar data
        os1 = self.get_lidar_data(sample['os1'])
        os2 = self.get_lidar_data(sample['os2'])

        # Save data
        os.makedirs(dst, exist_ok=True)

        # 保存VLM文本标签
        # with open(osp.join(dst, 'vlm_text_simple.txt'), 'w', encoding='utf-8') as f:
        #     f.write(vlm_text_simple)

        with open(osp.join(dst, 'vlm_text_detailed.txt'), 'w', encoding='utf-8') as f:
            f.write(vlm_text_detailed)

        # with open(osp.join(dst, 'vlm_data.json'), 'w', encoding='utf-8') as f:
        #     json.dump(vlm_data_json, f, indent=2, ensure_ascii=False)

        # 保存原有数据
        np.save(osp.join(dst, 'labels.npy'), boxes, allow_pickle=False)
        np.save(osp.join(dst, 'description.npy'), description_encoded, allow_pickle=False)
        cv2.imwrite(osp.join(dst, 'mono.jpg'), camera_front_left, self.jpg_quality)
        np.save(osp.join(dst, 'mono_info.npy'), mono_to_lidar, allow_pickle=False)
        cv2.imwrite(osp.join(dst, 'stereo.jpg'), camera_front_right, self.jpg_quality)
        np.save(osp.join(dst, 'stereo_info.npy'), stereo_to_lidar, allow_pickle=False)
    ...
    #
    #     Arguments:
    #         sequence: List of all sample path belonging to a single sequence.
    #         dst: Destination path to save the processed results of the sequence.
    #     """
    #     # 构建每个样本的数据路径字典，并提取场景描述
    #     sequence_paths = self.get_sequence_paths(sequence)
    #     description_tags = sequence_paths.pop('description')
    #
    #     # 只保留 Fog、Rain、Sleet、Light Snow、Heavy Snow
    #     climate = description_tags[2].strip().lower()
    #     valid_weathers = {'fog', 'rain', 'sleet', 'light_snow', 'heavy_snow'}
    #     if climate not in valid_weathers:
    #         return  # 非目标天气，跳过整个序列
    #
    #     # 并行处理序列中每个样本
    ...
    def generate_vlm_text(self, boxes: np.array, description: List[str],
                          include_bbox_details: bool = True) -> str:
        """将labels和description转化成包含3D边界框信息的VLM文本描述

        Arguments:
            boxes: Bounding boxes数组 (M, 9)
                   格式: [x, y, z, theta, l, w, h, category_idx, obj_id]
            description: 场景描述列表 [road_type, capture_time, climate]
            include_bbox_details: 是否包含详细的边界框坐标信息

        Returns:
            text: 适合VLM使用的文本描述
        """
        # 反向映射类别索引到类别名称
        idx_to_category = {v: k for k, v in self.categories.items()}

        # 解析场景描述
        road_type = description[0].replace('_', ' ')
        time_zone = description[1]
        weather = description[2].replace('_', ' ')

        # 构建场景描述
        scene_text = f"A {road_type} scene at {time_zone}"
        if weather != "normal":
            scene_text += f" with {weather}"
        scene_text += "."

        # 如果没有检测到物体
        if boxes.size == 0:
            return scene_text + " No objects detected."
        ...
        for idx, box in enumerate(boxes):
            x, y, z, theta, l, w, h, category_idx, obj_id = box
            category_idx = int(category_idx)

            # 跳过背景类别
            if category_idx < 0 or category_idx == 7:  # 7 是 Background
                continue

            category_name = idx_to_category.get(category_idx, "Unknown")
            object_counts[category_name] = object_counts.get(category_name, 0) + 1

            if include_bbox_details:
                # 计算距离（到原点的距离）
                ...
                # 构建详细描述
                obj_desc = (
                    f"{category_name.lower()} (ID:{int(obj_id)}) at "
                    f"position ({x:.2f}, {y:.2f}, {z:.2f})m, "
                    f"dimensions {l:.2f}x{w:.2f}x{h:.2f}m (LxWxH), "
        ...
        # 构建摘要文本
        if object_counts:
            count_text = ", ".join([f"{count} {name.lower()}"
                                    for name, count in sorted(object_counts.items())])
            summary_text = f" Detected {count_text}."
        else:
            summary_text = " No objects detected."

        # 组合完整文本
        if include_bbox_details and object_descriptions:
            detail_text = " Objects: " + "; ".join(object_descriptions) + "."
            full_text = scene_text + summary_text + detail_text
        else:
            full_text = scene_text + summary_text
