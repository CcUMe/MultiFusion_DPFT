...
class BalancedGroupSetCriterion(SetCriterion):
    ...
    @classmethod
    def from_config(cls, config: Dict[str, Any]) -> BalancedGroupSetCriterion:  # noqa: F821
    ...
    def objectness_loss(self,
        ...
        i, _ = indices
        if i.numel():
            target.scatter_(1, i, 1.0)
            weights.scatter_(1, i, 1.0)
    ...
    def bags_class_loss(self,
        ...
        gt_class = torch.argmax(targets['gt_class'][0, target_idx], dim=-1)
    ...
    def forward(
        ...
        losses = {
            # 背景前景区分损失
            "objectness": self.objectness_loss(inputs, targets, indices),
            # 分组损失
            "bags_class": self.bags_class_loss(inputs, targets, indices),
        }

        i, _ = indices
        for name in ("center", "size", "angle"):
            if i.numel():
                losses[name] = self.l1_loss(
                    self._dstack_dict(inputs, self.loss_inputs[name]),
                    self._dstack_dict(targets, [f"gt_{n}" for n in self.loss_inputs[name]]),
                    indices
                )
            else:
                losses[name] = inputs[name].sum() * 0.0

        return losses


class WeakBEVSetCriterion(BalancedGroupSetCriterion):
    """Set criterion for weak 2D/BEV labels.

    Supervises objectness, grouped class, 3D center, and BEV length/width.
    It intentionally ignores height and yaw because the weak label mode does
    not require those annotations.
    """
    ...
    @classmethod
    def from_config(cls, config: Dict[str, Any]) -> WeakBEVSetCriterion:  # noqa: F821
        model_head = config.get('model', {}).get('head', {})
        bags_config = model_head.get('bags', {})
        train_config = config.get('train', {})
        return cls(
    ...
    def size_lh_loss(self,
                     inputs: Dict[str, torch.Tensor],
                     targets: Dict[str, torch.Tensor],
                     indices: Tuple[torch.Tensor, torch.Tensor]) -> torch.Tensor:
        i, j = indices
        return F.l1_loss(
            self._batched_index_select(inputs['size'][..., (0, 2)], dim=1, inds=i),
            self._batched_index_select(targets['gt_size_lh'], dim=1, inds=j),
            reduction='mean'
        )
    ...
    def forward(
        ...
        i, _ = indices
        if i.numel():
            losses["center"] = self.l1_loss(
                self._dstack_dict(inputs, ["center"]),
                self._dstack_dict(targets, ["gt_center"]),
                indices
            )
            losses["size_lh"] = self.size_lh_loss(inputs, targets, indices)
        else:
            losses["center"] = inputs["center"].sum() * 0.0
            losses["size_lh"] = inputs["size"].sum() * 0.0
...
class Loss(nn.modules.loss._Loss):
    ...
    def from_config(cls, config: Dict[str, Any]) -> Loss:  # noqa: F821
        ...
        weak_mode = label_mode in {'weak', 'weak_2d', 'weak_2d_bev', 'weak_center_lh'}
        anassigner = None
        criterion = None
        losses = None
        loss_inputs = train_config.get('loss_inputs')
        if weak_mode:
            loss_weights = train_config.get('weak_loss_weights', {
                'objectness': 1.0,
                'bags_class': 1.0,
                'center': 1.0,
                'size_lh': 1.0,
        ...
        reduction = train_config.get('reduction', 'mean')

        if weak_mode:
            anassigner = build_anassigner(
                train_config.get('weak_anassigner', 'WeakBEVHungarianAnassigner'),
                train_config
            )
        elif 'anassigner' in train_config:
            anassigner = build_anassigner(train_config['anassigner'], train_config)

        if weak_mode:
            criterion = _get_loss(
                train_config.get('weak_criterion', 'WeakBEVSetCriterion'),
                config
            )
        elif 'criterion' in train_config:
            criterion = _get_loss(train_config['criterion'], config)

        if 'losses' in train_config:
            losses = {k: _get_loss(v) for k, v in train_config['losses'].items()}
    ...
    def forward(
        ...
        for input, target in zip(inputs, targets):
            ...
            if not all([t.numel() for t in target.values()]) and \
                    not getattr(self.criterion, 'allow_empty_targets', False):
                batch_losses.append({
        ...
        if not self.losses and self.criterion is None:
...
def _get_loss(name: str, config: Dict[str, Any] = None) -> nn.modules.loss._Loss:
    ...
    try:
        ...
        loss_cls = globals()[name]
        if config is not None and hasattr(loss_cls, 'from_config'):
