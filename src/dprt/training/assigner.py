...
# 简化的匈牙利分配器，直接处理边界框坐标
class WeakBEVHungarianAnassigner(nn.Module):
    """Hungarian matching for weak 2D/BEV supervision."""
    ...
    @classmethod
    def from_config(cls, config: Dict[str, Any]) -> WeakBEVHungarianAnassigner:  # noqa: F821
        return cls(loss_weights=config.get('weak_loss_weights', config.get('loss_weights')))
    ...
    def forward(self,
        ...
        with torch.no_grad():
            ...
            out_class = outputs["class"].flatten(0, 1)
            out_center = outputs["center"].flatten(0, 1)
            out_size_lh = outputs["size"][..., (0, 2)].flatten(0, 1)

            gt_class = targets["gt_class"].flatten(0, 1)
            gt_center = targets["gt_center"].flatten(0, 1)
            gt_size_lh = targets["gt_size_lh"].flatten(0, 1)
            gt_ids = torch.argmax(gt_class, dim=-1)

            cost_class = -out_class[:, gt_ids]
            cost_center = torch.cdist(out_center, gt_center, p=1)
            cost_size_lh = torch.cdist(out_size_lh, gt_size_lh, p=1)
            ...
            C = class_weight * cost_class \
                + self.loss_weights.get('center', 1.0) * cost_center \
                + self.loss_weights.get('size_lh', self.loss_weights.get('size', 1.0)) * cost_size_lh
...
def build_anassigner(name: str, *args, **kwargs):
    if 'weak' in name.lower() or 'bev' in name.lower():
        return WeakBEVHungarianAnassigner.from_config(*args, **kwargs)
