...
class ModalityLevelDynamics(nn.Module):
    def __init__(self, hidden_dim=64):
        ...
        # 关键修改1：保持标准的第一个卷积层，适配3通道输入
        original_conv1 = backbone.conv1
        self.conv1 = nn.Conv2d(
            in_channels=3,  # 保持3输入通道（RGB图像）
    ...
    def forward(self, x, model_selectors_lidar_bev = None):
        """
        前向传播 - 使用多尺度特征
        
        参数：
            x: 3通道激光雷达 tensor, shape=(batch_size, H, W, 3)
            model_selectors_camera: 稀疏特征选择器
        """
        # 调整维度: [B, H, W, 3] -> [B, 3, H, W]
        x = x.permute(0, 3, 1, 2)
        ...
        feat4 = self.layer4(feat3)  # [B, 2048, H4, W4]
         ...
         # 应用稀疏特征选择器（如果提供）
        if model_selectors_lidar_bev is not None:
            ...
            # 应用稀疏选择器
            selector_outputs = model_selectors_lidar_bev(multi_scale_features, training=False)
