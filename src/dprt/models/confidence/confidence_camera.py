...
class ModalityLevelDynamics(nn.Module):
    ...
    def forward(self, x, model_selectors_camera=None):
        """
        前向传播 - 使用多尺度特征
        
        参数：
            x: 3通道可见光图像 tensor, shape=(batch_size, H, W, 3)
        ...
        feat4 = self.layer4(feat3)  # [B, 2048, H4, W4]
         ...
         # 应用稀疏特征选择器（如果提供）
        if model_selectors_camera is not None:
            ...
            # 应用稀疏选择器
            selector_outputs = model_selectors_camera(multi_scale_features, training=False)
