...
class WeightedFusion(nn.Module):
    ...
    def __init__(self, d_model: int, m_views: int, fusion_type: str = 'gated'):
        ...
        if fusion_type == 'gated':
            ...
            # SAMFusion 式: 成对交叉注意力 + 权重池化
            self.cross_cam_lidar = nn.MultiheadAttention(d_model, num_heads=8, dropout=0.1, batch_first=True)
            self.cross_lidar_cam = nn.MultiheadAttention(d_model, num_heads=8, dropout=0.1, batch_first=True)
            self.cross_cam_radar = nn.MultiheadAttention(d_model, num_heads=8, dropout=0.1, batch_first=True)
            self.norm = nn.LayerNorm(d_model)
            self.modality_pos = nn.Parameter(torch.randn(m_views, d_model) * 0.02)
            self.weight_mlp = nn.Sequential(
                nn.Linear(m_views, m_views // 2),
                nn.ReLU(),
                nn.Dropout(0.1),
                nn.Linear(m_views // 2, m_views),
                nn.Softmax(dim=-1)
            )
            self.mod_dropout = nn.Dropout(0.1)
    ...
    def forward(self, query: torch.Tensor, queries: torch.Tensor) -> torch.Tensor:
        ...
        if self.fusion_type == 'gated':
            ...
            concat_queries = queries.view(B, N, d_model * m_views)
            return self.fusion_mlp(concat_queries) + query

        elif self.fusion_type == 'attention':
            modal_feats = queries.view(B * N, m_views, self.d_model)
            modal_pos   = self.modality_pos.unsqueeze(0).expand(B * N, -1, -1)
            modal_feats = modal_feats + modal_pos
            fused_seq, _ = self.cross_attn(
                query=modal_feats, key=modal_feats, value=modal_feats
            )
            fused = fused_seq.mean(dim=1).view(B, N, self.d_model)
            return self.norm(fused + query)

        elif self.fusion_type == 'sam_cross':
            f_cam   = queries[..., 0]
            f_lidar = queries[..., 3]
            f_radar = torch.mean(queries[..., 1:3], dim=-1)

            c_enh, _ = self.cross_cam_lidar(query=f_cam,   key=f_lidar, value=f_lidar)
            c_enh    = self.norm(c_enh + f_cam)
            l_enh, _ = self.cross_lidar_cam(query=f_lidar, key=c_enh,   value=c_enh)
            l_enh    = self.norm(l_enh + f_lidar)
            r_enh, _ = self.cross_cam_radar(query=c_enh,   key=f_radar, value=f_radar)
            r_enh    = self.norm(r_enh + f_radar)

            all_feats = torch.stack([c_enh, l_enh, r_enh, f_radar], dim=-1)
            feat_mean = torch.mean(all_feats, dim=-2)        # (B, N, 4)
            weights   = self.weight_mlp(feat_mean)           # (B, N, 4)
            weights   = weights.unsqueeze(-2)                # (B, N, 1, 4)
            fused     = torch.sum(all_feats * weights, dim=-1)
            fused     = self.mod_dropout(fused)
            return self.norm(fused + query)
            ...
            f_fusion       = torch.cat([f_lidar, f_radar], dim=-1)
            query_pos_feat = self.query_pos_proj(query)
            fusion_guide   = self.cross_guide_net(f_fusion) + query_pos_feat

            spatial_gates    = self.spatial_gate(fusion_guide)
            gate_mask        = (spatial_gates > self.gate_threshold).float()
            spatial_gates    = spatial_gates * gate_mask
            modality_weights = self.modality_weight(fusion_guide)
            combined_gates   = spatial_gates * modality_weights
            gate_sum         = combined_gates.sum(dim=-1, keepdim=True) + 1e-8
            combined_gates   = combined_gates / gate_sum
            combined_gates   = combined_gates.unsqueeze(2)   # (B, N, 1, m_views)
            return torch.sum(queries * combined_gates, dim=-1) + query
...
class MLFusion(nn.Module):
    def __init__(self,
        ...
        Arguments:
            d_model:    Hidden feature (channel) dimension of the attention modules.
            d_ffn:      Hidden feature (channel) dimension of the feed forward module.
            n_levels:   Number of feature levels of the cross attention module.
    ...
    def forward_cross_attn(self,
        ...
        input_flatten = torch.cat(
            tuple(self.flatten1(l) for l in batch.values()), dim=1
        )
        input_level_start_index = torch.cumsum(
            torch.as_tensor(
                [0] + [l.shape[1] * l.shape[2] for l in batch.values()],
                device=query.device
            ),
            dim=0
        )[:-1]
...
class MPFusion(nn.Module):
    def __init__(self,
        ...
        _valid_reductions = {
            'mean', 'max', 'unary', 'linear', 'cross-attn', 'ffn',
            'weighted', 'concat_weighted', 'attn_weighted',
            'sam_cross_pool', 'enhanced_gated'
        }
        if reduction not in _valid_reductions:
            raise ValueError(
                f"reduction must be one of {_valid_reductions}, got '{reduction}'"
            )
        ...
        self.ml_fusion_layers = nn.ModuleDict({
            ...
            MLFusion(self.d_model, self.d_ffn, l, h, p,
                     self.ffn_layer, self.activation, self.dropout, self.norm)
            for v, l, h, p in zip(
                range(self.m_views), self.n_levels, self.n_heads, self.n_points
            )
        })
    ...
    def _init_reduction(self, reduction: str) -> Union[Callable, nn.Module]:
        ...
        if reduction == 'unary':
            return layers.Unary1d(in_channels=self.m_views * self.d_model,
                                  ...
                                  bias=False, channels_last=True)
        if reduction == 'linear':
            return nn.Linear(in_features=self.m_views * self.d_model,
                             out_features=self.d_model, bias=False)
        if reduction == 'cross-attn':
            return nn.MultiheadAttention(self.d_model, min(self.n_heads),
                                         dropout=self.dropout,
                                         kdim=self.d_model * self.m_views,
                                         vdim=self.d_model * self.m_views,
                                         batch_first=True)
        if reduction == 'ffn':
            return nn.ModuleDict({
                'ffn1':        self._get_ffn_layer(self.ffn_layer,
                                                   self.m_views * self.d_model,
                                                   self.m_views * self.d_model),
                'activation1': self._get_activation_fn(self.activation),
                'dropout1':    nn.Dropout(self.dropout),
                'ffn2':        self._get_ffn_layer(self.ffn_layer,
                                                   self.m_views * self.d_model,
                                                   self.d_model),
                'downsample1': self._get_ffn_layer(self.ffn_layer,
                                                   self.m_views * self.d_model,
                                                   self.d_model),
                'dropout2':    nn.Dropout(self.dropout),
                'norm1':       nn.LayerNorm(self.d_model),
    ...
    def forward(self,
                ...
                query_positions: torch.Tensor) -> torch.Tensor:
        queries  = torch.zeros(
            query.shape + (self.m_views,),
            dtype=query.dtype, device=query.device
        )
        iterator = zip(self.ml_fusion_layers.values(), batch, reference_points)
        for i, (ml_fusion_layer, levels, ref_points) in enumerate(iterator):
            queries[..., i] = ml_fusion_layer(query, levels, ref_points, query_positions)

        return self.reduce(query, queries, query_positions)


# ═══════════════════════════════════════════════════════════════════
# ProjectionHead  （对齐 VLM 维度，不修改）
# ═══════════════════════════════════════════════════════════════════

class ProjectionHead(nn.Module):
    def __init__(self, in_dim: int = 16, out_dim: int = 2560,
...
class IMPFusion(nn.Module):
    def __init__(self,
                 ...
                 head: nn.Module = None,
                 # ── GPQ 新增参数 ──────────────────────────────────
                 gpq_prune_ratio: float = 0.0,
                 gpq_min_queries: int = 10,
                 # ─────────────────────────────────────────────────
                 **kwargs):
        """Iterative Multi-Perspective Fusion Transformer with GPQ.
        ...
        Arguments:
            ...
            n_levels:         每视角的特征层数列表。
            n_heads:          每视角的注意力头数列表。
            n_points:         每视角每头的采样点数列表。
            q_init:           query 参数初始化方法（nn.init.*）。
            ffn_layer:        FFN 层类型。
            activation:       激活函数类型。
            dropout:          Dropout 比率。
            norm:             是否使用 LayerNorm。
            reduction:        多视角融合方式。
            head:             检测头模块。
            gpq_prune_ratio:  GPQ —— 每轮迭代后裁剪的 query 比例。
                              0.0 表示禁用 GPQ（等价于原始 IMPFusion）。
                              例如 0.3 表示每轮保留 70% 的 query。
            gpq_min_queries:  GPQ —— 任意迭代后保留的最少 query 数，
                              防止过度裁剪。
        """
        super().__init__()
        ...
        # ── GPQ：预计算每轮迭代后应保留的 query 数 ───────────────
        #   gpq_n_keep 的长度 = i_iter - 1（最后一轮不裁剪）
        #   gpq_n_keep[t] 表示第 t 轮（0-indexed）结束后保留的数量
        self.gpq_prune_ratio = gpq_prune_ratio
        self.gpq_min_queries = gpq_min_queries

        if gpq_prune_ratio > 0.0 and i_iter > 1:
            n_keep_list: List[int] = []
            n = n_queries
            for _ in range(i_iter - 1):   # 最后一轮不裁剪，故只算 i_iter-1 次
                n = max(gpq_min_queries, int(n * (1.0 - gpq_prune_ratio)))
                n_keep_list.append(n)
            self.gpq_n_keep: Optional[List[int]] = n_keep_list
        else:
            self.gpq_n_keep = None  # GPQ 已禁用
    ...
    def _gpq_prune(
        ...
        """GPQ：依据分类置信度保留 top-k 个 query，裁剪其余 query。

        裁剪后：
          - query     shape: (B, n_keep, d_model)
          - query_pos shape: (B, n_keep, d_model)
          - out 各项  shape: (B, n_keep, *)

        注意：
          - scores 计算使用 .detach()，避免 top-k 操作干扰梯度。
          - out['center'] 同步裁剪，保证下一轮 get_reference_points
            的输入与 query 一一对应。

        Arguments:
            query:     当前查询特征 (B, N, d_model)
            query_pos: 查询位置编码 (B, N, d_model)
            out:       当前迭代的输出字典，至少含 'class' 键 (B, N, C_cls)
            n_keep:    本轮保留的 query 数量

        Returns:
            pruned_query, pruned_query_pos, pruned_out
        """
        # 1. 计算每个 query 的置信度分数：各类别 sigmoid 后取最大值
        scores = out['class'].detach().sigmoid().max(dim=-1).values  # (B, N)

        # 2. top-k 索引（降序）
        _, topk_idx = scores.topk(n_keep, dim=-1, largest=True, sorted=False)
        # topk_idx: (B, n_keep)

        # 3. 裁剪 query 特征
        idx_q        = topk_idx.unsqueeze(-1).expand(-1, -1, query.shape[-1])
        pruned_query = torch.gather(query, dim=1, index=idx_q)      # (B, n_keep, d_model)

        # 4. 裁剪位置编码
        idx_p            = topk_idx.unsqueeze(-1).expand(-1, -1, query_pos.shape[-1])
        pruned_query_pos = torch.gather(query_pos, dim=1, index=idx_p)

        # 5. 裁剪 out 字典中所有预测张量（class / center / size / angle 等）
        pruned_out = {}
        for k, v in out.items():
            idx_v        = topk_idx.unsqueeze(-1).expand(-1, -1, v.shape[-1])
            pruned_out[k] = torch.gather(v, dim=1, index=idx_v)

        return pruned_query, pruned_query_pos, pruned_out
    ...
    def get_reference_points(self,
                             ...
                             projection: torch.Tensor,
                             shape: torch.Size) -> torch.Tensor:
        """Returns the query reference points in the feature space.

        Projects query points (X, Y, Z) to feature space (u, v)
        using the given projection matrix.

        Arguments:
            query:          Query points (B, N, 3).
            transformation: 4×4 homogeneous transformation matrix (B, 4, 4).
            projection:     4×4 homogeneous projection matrix (B, 4, 4).
            shape:          Feature map shape (B, 2).

        Returns:
            reference_points: (B, N, 2), values in [0, 1].
        """
        if transformation.any():
            reference_points = torch.einsum(
                'bij,bkj->bki',
        ...
        reference_points = torch.einsum(
            'bij,bkj->bki',
            projection,
            torch.dstack((reference_points[..., :3],
                          torch.ones_like(reference_points[..., 0])))
        )
    ...
    def forward(self,
                ...
                projection: List[Tuple[torch.Tensor, torch.Tensor]],
                out: Dict[str, torch.Tensor]) -> torch.Tensor:
        """Iteratively fuses multi-perspective features with optional GPQ pruning.

        GPQ 行为说明
        ─────────────
        当 gpq_prune_ratio > 0 且 i_iter > 1 时：
          - 在第 t 轮（0-indexed）迭代结束后（t < i_iter-1），
            依据检测头输出的分类置信度对 query 做 top-k 筛选，
            保留 gpq_n_keep[t] 个高分 query。
          - 最后一轮（t == i_iter-1）不裁剪，保持全量输出。
          - 裁剪同步作用于 query、query_pos、out 字典，
            确保 center 与 query 一一对应。

        当 gpq_prune_ratio == 0 时，行为与原始 IMPFusion 完全一致。

        Arguments:
            batch:      多视角特征字典列表，长度 m_views，每项 dict 长度 n_levels。
            shape:      各视角原始输入尺寸列表，每项 (B, 2)。
            projection: 各视角投影矩阵列表，每项为 (T, P) 两个 (B,4,4) 矩阵的元组。
            out:        初始输出字典，至少含 'center': (B, N, 3)。

        Returns:
            out: 最终输出字典，含 class / center / size / angle。
        """
        B = out['center'].shape[0]

        # ── 初始化 query 和位置编码 ───────────────────────────────
        query     = self.query.unsqueeze(0).repeat(B, 1, 1)                    # (B, N, d_model)
        query_pos = self.query_embedding.weight.unsqueeze(0).repeat(B, 1, 1)   # (B, N, d_model)

        # ── 迭代融合 ──────────────────────────────────────────────
        for iter_idx, (layer, head) in enumerate(
            zip(self.mpfusion.values(), self.heads)
        ):
            # 1. 基于上一轮 center 计算各视角参考点
            reference_points = [
                self.get_reference_points(out['center'][..., :3], p[0], p[1], s)
                for p, s in zip(projection, shape)
            ]
            ...
            # ── GPQ 裁剪（仅在非最后一轮 && 已启用 && out 含 class） ──
            if (
                self.gpq_n_keep is not None
                and iter_idx < len(self.gpq_n_keep)   # 非最后一轮
                and 'class' in out                     # 检测头已输出分类分数
            ):
                n_keep = self.gpq_n_keep[iter_idx]
                query, query_pos, out = self._gpq_prune(
                    query, query_pos, out, n_keep
                )
