...
class IMPFusion(nn.Module):
    def __init__(self,
        ...
        参数说明：
            ...
            early_exit_from_iter: 从第几轮开始允许退出（0-based）
        ...
        self.early_exit_from_iter = early_exit_from_iter
        ...
        # 如果用户给的是 keep ratio，则自动转换成 keep count
        # 例如 n_queries=400, stage_keep_ratios=[0.4, 0.2]
        # -> stage_keep_counts=[160, 80]
        if self.stage_keep_counts is None and self.stage_keep_ratios is not None:
            self.stage_keep_counts = [
                max(1, int(round(self.n_queries * ratio))) for ratio in self.stage_keep_ratios
            ]
        # 如果都没给，则默认每轮折半
        elif self.stage_keep_counts is None:
            self.stage_keep_counts = [
                max(1, self.n_queries // (2 ** (i + 1))) for i in range(max(self.i_iter - 1, 0))
            ]

        # 如果长度不够，自动用最后一个值补齐
        # 比如 i_iter=4，需要 3 个 keep_count，但只给了 2 个，就补成 [160, 80, 80]
        if len(self.stage_keep_counts) < max(self.i_iter - 1, 0):
            tail = self.stage_keep_counts[-1] if self.stage_keep_counts else max(1, self.n_queries // 2)
            self.stage_keep_counts = self.stage_keep_counts + [tail] * (
                max(self.i_iter - 1, 0) - len(self.stage_keep_counts)
            )
    ...
    def _get_query_scores(self, out: Dict[str, torch.Tensor]) -> torch.Tensor:
        """
        从 out 中取出每个 query 的得分。

        默认使用 out['class']，如果是多类别输出：
            shape = (B, N, num_classes)
        则取最后一维最大值作为该 query 的目标性分数。

        返回：
            scores: (B, N)
        """
        if self.score_key not in out:
            raise KeyError(f"Expected score key '{self.score_key}' in output dict, but found {list(out.keys())}")
        scores = out[self.score_key]
        if scores.ndim == 2:
            return scores
    ...
    def _compute_exit_mask(self,
        ...
        返回：
            exit_mask: (B, N)，True 表示该 query 提前退出，不再参加后续 refinement
        """
        if prev_out is None:
        ...
        stable = (
            (center_delta <= self.exit_center_threshold) &
            (size_delta <= self.exit_size_threshold) &
    ...
    def _append_to_final_storage(storage: List[Dict[str, Any]],
        ...
        for b in range(B):
            ...
            for key, value in out.items():
                ...
                storage[b]['fields'][key].append(value[b][mask].detach())
    ...
    def forward(self,
        ...
        for i, (layer, head) in enumerate(zip(self.mpfusion.values(), self.heads)):
            ...
            # 2) query early exit
            # -----------------------------
            if self.early_exit and i >= self.early_exit_from_iter:
                # 哪些 query 已经高置信且足够稳定，可以提前退出
                exit_mask = self._compute_exit_mask(out, prev_out, scores, valid_mask)
                ...
                if continue_mask.sum().item() == 0:
                    return self._build_final_output(final_storage, out)
