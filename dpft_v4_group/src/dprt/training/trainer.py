...
class CentralizedTrainer():
    ...
    def _to_float(v: Any) -> float:
        """Used for tqdm display: tensor -> float."""
    ...
    def train_one_epoch(self, epoch: int, model: torch.nn.Module,
        ...
        for i, (data, labels) in enumerate(data_loader):
            ...
            # Evaluate model output (contains mAP if configured)
    ...
    def validate_one_epoch(self, epoch: int, model: torch.nn.Module, data_loader: Iterable,
        ...
        # Average epoch logs (always, so we can return mAP/loss for display)
        scalars = {k: v / (i + 1) for k, v in scalars.items()}
        ...
        # Return all scalars so tqdm can show loss/mAP
        return scalars
    ...
    def train(self, model: torch.nn.Module, data_loader: Iterable, val_loader: Iterable = None,
        ...
        for epoch in tbar:
            ...
            # Show only loss and overall mAP in progress bar (if present)
            postfix = {}
            if 'loss' in result:
                postfix['loss'] = self._to_float(result['loss'])
            if 'mAP' in result:
                postfix['mAP'] = self._to_float(result['mAP'])
            if postfix:
                tbar.set_postfix(postfix, refresh=True)

            # Save checkpoint for every epoch
            if dst is not None:
                epoch_path = osp.join(dst, timestamp, 'checkpoints',
                            f"{timestamp}_checkpoint_{str(epoch).zfill(4)}.pt")
                torch.save(model, epoch_path)
                print(f"[✓] Epoch {epoch + 1} saved: {epoch_path}")

                # Also save best mAP checkpoint (requires val + mAP)
                if 'mAP' in result:
                    cur_map = self._to_float(result['mAP'])
                    if cur_map > best_map:
                        best_map = cur_map
                        best_path = osp.join(dst, timestamp, 'checkpoints', 'best.pt')
                        torch.save(model, best_path)
                        print(f"[✓] New best mAP: {best_map:.6f} | saved: {best_path}")
